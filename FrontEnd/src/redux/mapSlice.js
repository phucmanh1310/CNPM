...
function handleFamousLandmarks(address) {
    const landmarks = {
        ...
        'đại học bách khoa': '268 Lý Thường Kiệt, Phường 14, Quận 10, TP. Hồ Chí Minh',
        'đại học kinh tế': '59C Nguyễn Đình Chiểu, Phường 6, Quận 3, TP. Hồ Chí Minh',
        ...
        'landmark 81': 'Vinhomes Central Park, Phường 22, Bình Thạnh, TP. Hồ Chí Minh',
        'bitexco': '2 Hải Triều, Phường Bến Nghé, Quận 1, TP. Hồ Chí Minh',
        'aeon mall': '1 Đường số 17A, Phường Bình Trị Đông B, Bình Tân, TP. Hồ Chí Minh',
        'vincom center': '72 Lê Thánh Tôn, Phường Bến Nghé, Quận 1, TP. Hồ Chí Minh',
        'lotteria': 'Lotte Mart, 469 Nguyễn Hữu Thọ, Phường Tân Hưng, Quận 7, TP. Hồ Chí Minh',
                ...
                async (position) => {
                    ...
                    try {
                        ...
                        if (response.data.features.length > 0) {
                            ...
                            resolve({ lat: roundedCoords.lat, lon: roundedCoords.lon, address });
                        } else {
                            const roundedCoords = roundCoordinates(latitude, longitude);
                            resolve({ lat: roundedCoords.lat, lon: roundedCoords.lon, address: "Không xác định được địa chỉ" });
                        }
                    } catch (error) {
        ...
        try {
            ...
            if (response.data.features.length > 0) {
                ...
                const bestResult = response.data.features.reduce((best, current) =>
                    current.properties.rank.confidence > best.properties.rank.confidence ? current : best
                );
...
export const reverseGeocodeFromCoordinates = createAsyncThunk(
    ...
    async ({ lat, lon }) => {
        try {
            ...
            const roundedCoords = roundCoordinates(lat, lon);
            return { lat: roundedCoords.lat, lon: roundedCoords.lon, address: "Không xác định được địa chỉ" };
        } catch (error) {
            console.log(error.message)
            const roundedCoords = roundCoordinates(lat, lon);
            return { lat: roundedCoords.lat, lon: roundedCoords.lon, address: "Không thể lấy địa chỉ" };
        }
    }
);
...
export const { setLocation, setAddress, clearSuggestions, clearError } = mapSlice.actions;
