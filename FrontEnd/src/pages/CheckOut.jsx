...
function MapUpdater({ center, zoom }) {
  ...
  }, [center, center.lat, center.lon, zoom, map])
        ...
        try {
          ...
          if (paymentResult.data.success) {
            ...
            localStorage.setItem(
              'lastPaymentId',
              paymentResult.data.data.paymentId
            )
          ...
          console.error(
            'Payment error:',
            paymentError.response?.data || paymentError
          )
          alert(
            `Payment error: ${paymentError.response?.data?.message || 'Unknown error'}`
          )
        }
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#fff9f6] flex items-center justify-center p-6">
      ...
      <div className="w-full max-w-[900px] bg-white rounded-2xl shadow-lg p-6 space-y-6">
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <input
                  ...
                  onFocus={() => {
                    if (inputAddress) debouncedSuggest(inputAddress)
                  }}
                />
                <IoSearchOutline
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div
              className={`flex items-center gap-3 rounded-xl border p-4 text-left transition 
                        ${
                          paymentMethod === 'cod'
                            ? 'border-[#00BFFF] bg-blue-50 shadow'
        ...
        {/*Submit button*/}
        <button
          className="w-full bg-[#00BFFF] text-white py-3 rounded-md font-semibold hover:bg-blue-600 transition"
