...
function AdminDashboard() {
    ...
    const handleUserToggle = async (userId, isActive) => {
        try {
            await axios.patch(`${serverURL}/api/admin/users/${userId}/toggle`,
                { isActive: !isActive },
                { withCredentials: true }
            );
    ...
    const handleShopToggle = async (shopId, isActive) => {
        try {
            await axios.patch(`${serverURL}/api/admin/shops/${shopId}/toggle`,
                { isActive: !isActive },
                { withCredentials: true }
            );
            fetchData(); // Refresh data
                                        ...
                                        <tr key={user._id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <button
                                                    ...
                                                    className={`px-3 py-1 text-xs font-semibold rounded-full ${user.isActive !== false
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {shops.map((shop) => (
                                        <tr key={shop._id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${shop.isActive !== false ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                                    }`}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <button
                                                    onClick={() => handleShopToggle(shop._id, shop.isActive)}
                                                    className={`px-3 py-1 text-xs font-semibold rounded-full ${shop.isActive !== false
                                                        ? 'bg-red-100 text-red-800 hover:bg-red-200'
                                                        : 'bg-green-100 text-green-800 hover:bg-green-200'
                                                        }`}
                                                >
