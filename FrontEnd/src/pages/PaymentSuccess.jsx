    ...
    useEffect(() => {
        const checkPaymentStatus = async () => {
            try {
                ...
                console.log('Payment ID from localStorage:', localStorage.getItem('lastPaymentId'));
                ...
                const response = await axios.get(`${serverURL}/api/payment/status/${paymentId}`, {
                ...
                {paymentData && (
                    <div className="bg-gray-50 rounded-lg p-4 mb-6 text-left">
                        ...
                        <div className="space-y-1 text-sm text-gray-600">
                            ...
                            <div className="flex justify-between">
                                <span>Status:</span>
                                <span className={`font-medium capitalize ${paymentData.paymentStatus === 'success' ? 'text-green-600' :
                                    paymentData.paymentStatus === 'failed' ? 'text-red-600' : 'text-yellow-600'
                                    }`}>
                            ...
                            {paymentData.momoTransactionId && (
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-xs">{paymentData.momoTransactionId}</span>
                                </div>
                            )}
                        </div>
                    </div>
