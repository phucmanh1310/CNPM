...
function MyOrders() {
    ...
    const { userOrders, loading: userLoading, refetch: refetchUserOrders } = useGetUserOrders();
    const { ownerOrders, loading: ownerLoading, refetch: refetchOwnerOrders } = useGetOwnerOrders();
    const { confirmDelivery, loading: confirmLoading } = useConfirmDelivery();
    const { notifications, addNotification, removeNotification } = useOrderNotifications();
    ...
    const handleStatusUpdate = async (orderId, shopOrderId, newStatus) => {
        try {
            setUpdating(true);
            await axios.put(`${serverURL}/api/order/updateOrderStatus`, {
                orderId,
                shopOrderId,
                status: newStatus
            ...
            setToast({ show: true, message: 'Update order status to cancelled', type: 'error' });
        } finally {
            setUpdating(false);
        }
    };
    ...
    const handleCancelOrder = async (orderId, shopOrderId, cancelReason) => {
        try {
            setUpdating(true);
            await axios.put(`${serverURL}/api/order/cancelOrder`, {
                orderId,
                shopOrderId,
                cancelReason
            }, { withCredentials: true });
            ...
            setToast({ show: true, message: 'The order has been canceled', type: 'success' });
        } catch (error) {
            console.error('Error cancelling order:', error);
            setToast({ show: true, message: error.response?.data?.message || 'Order cancellation failed', type: 'error' });
        } finally {
            setUpdating(false);
        }
    };
