...
function DroneCard({
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'Under Maintenance':
        return <FaCog className="text-red-600" />
      default:
        return <FaPlane className="text-gray-600" />
    }
  }
  ...
  return (
    <div
      className={`bg-white rounded-lg border-2 p-4 shadow-sm hover:shadow-md transition-all duration-200 ${getStatusColor(drone.status)}`}
    >
      ...
      {drone.status === 'Busy' && drone.assignedOrderId && (
        <div className="mb-3 p-2 bg-blue-50 rounded-lg">
          <p className="text-sm text-blue-700">
            <strong>Assigned to Order:</strong> #
            {drone.assignedOrderId.slice(-8)}
          </p>
        </div>
      )}
