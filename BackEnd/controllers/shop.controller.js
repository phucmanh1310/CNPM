...
export const creatEditShop = async (req, res) => {
  try {
    ...
    const shopData = { name, city, state, address, owner: req.userId }
    if (image) {
    ...
    return res
      .status(500)
      .json({ message: `create shop controller ${error.message}` })
  }
}
...
export const getShopByCity = async (req, res) => {
  try {
    ...
    console.log(
      'Shops data:',
      shops.map((s) => ({ name: s.name, city: s.city, state: s.state }))
    )
