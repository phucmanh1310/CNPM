...
const cleanupDuplicatePayments = async () => {
    try {
        ...
        // Process each group
        for (const [momoOrderId, paymentGroup] of Object.entries(groupedByMomoOrderId)) {
            if (paymentGroup.length > 1) {
                ...
                const [keepPayment, ...duplicatePayments] = paymentGroup.sort((a, b) => a.createdAt - b.createdAt);

                console.log(`Keeping payment: ${keepPayment._id} (Order: ${keepPayment.order._id})`);

                // Update duplicate payments to failed
                for (const duplicatePayment of duplicatePayments) {
                    console.log(`Marking as failed: ${duplicatePayment._id} (Order: ${duplicatePayment.order._id})`);
